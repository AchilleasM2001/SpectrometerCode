...
DEFAULT_CALIBRATION = ((355, 532), (577, 650))
...
class VideoProcessor:
    def __init__(self, calibration=DEFAULT_CALIBRATION, video_source=0):
        self.vid = MyVideoCapture(calibration, video_source)
        self.all_data = []  # List to store all measurements
        self.wavelengths = None  # Placeholder for wavelength values

    def run(self):
        ret, graphdata = self.vid.get_graph()
        if ret:
            if self.wavelengths is None:
                self.wavelengths = graphdata[1]  # Store wavelengths from the first measurement
            self.save_data(graphdata[2])  # Only save the intensity data
            print("Snapshot captured and data saved.")
            self.vid.__del__()

    def save_data(self, intensity_data):
        self.all_data.append(intensity_data)

    def compute_and_save_mean(self):
        # Read the existing intensity values from the CSV
        self.reference_intensity = pd.read_csv('Intensity.csv')['Intensity'].values
        
        mean_values = np.mean(self.all_data, axis=0)  # Compute mean across all measurements
        # Compute mean divided by the reference intensity
        ratio_values = np.divide(mean_values, self.reference_intensity)
        ratio_values = np.maximum(ratio_values, np.finfo(float).eps)  # Ensure no values are zero or negative
        log_ratio_values = -np.log10(ratio_values)  # Compute log10 of the ratio

        #now = time.strftime("%d-%m-%Y-%H-%M-%S")
        csv_filename = f"log10_ratio_1.csv"
        with open(csv_filename, 'w') as f:
            f.write('Wavelength,Log10MeanIntensityRatio\r\n')
            for wavelength, log_ratio_value in zip(self.wavelengths, log_ratio_values):
                f.write(f"{wavelength},{log_ratio_value}\r\n")
        print("Log10 of mean intensity ratio data saved to:", csv_filename)

if __name__ == '__main__':
    for i in range(1, 6):
        print(i)
        video_processor = VideoProcessor()  # Create a single instance
        video_processor.run()
    video_processor.compute_and_save_mean()  # Compute and save mean after all measurements
    sys.exit()
